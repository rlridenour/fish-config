function lualecture
    ...
    cd {$argv}
    touch {$argv}-data.org
    echo -e '#+startup: beamer' \n'#+TITLE: '\n'#+SUBTITLE: '\n'#+AUTHOR: Dr. Randy Ridenour'\n'#+BEAMER_HEADER: \institute{Department of Philosophy}'\n'#+DATE:'\n'#+BEAMER_THEME: obulua'>>{$argv}-data.org
    touch {$argv}-slides.org
    cat /Users/rlridenour/Dropbox/emacs/beamer/lualecture-beamer.org >{$argv}-slides.org
    echo -e '#+include: "'{$argv}'-data.org" :minlevel 1' >>{$argv}-slides.org
    touch {$argv}-notes.org
    cat /Users/rlridenour/Dropbox/emacs/beamer/lualecture-notes.org >{$argv}-notes.org
